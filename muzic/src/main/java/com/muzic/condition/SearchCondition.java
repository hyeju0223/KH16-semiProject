...
public class SearchCondition {
    ...
    public String getAllParams() {
        // 여기서는 예시로 keyword와 column만 포함했지만,
        // 실제 SearchCondition이 가진 모든 검색 관련 필드를 &로 연결해야 합니다.
        
        StringBuilder params = new StringBuilder();

        // 1. keyword가 있다면 추가
        if (this.keyword != null && !this.keyword.trim().isEmpty()) {
            params.append("column=").append(this.column).append("&");
            params.append("keyword=").append(this.keyword).append("&");
        }
        
        // 2. 다른 검색 조건이 있다면 여기에 추가 (예: size, category 등)
